...
( function ( $, mw ) {
	...
	var targetTitleWidget, loadBtn, previewBtn, closePreviewBtn, publishBtn;
	...
	function buildTargetPanel() {
		...
		closePreviewBtn = new OO.ui.ButtonWidget( {
			label: mw.msg( 'extendedinputbox-formbuilder-btn-closepreview' ),
			icon: 'close'
		} ).on( 'click', function () {
			$previewBox.empty();
			closePreviewBtn.toggle( false );
		...
		var $actionRow =$( '<div>' ).addClass( 'eib-fb-toolbar' ).append(
			insertBtn.$element, previewBtn.$element, closePreviewBtn.$element, publishBtn.$element
		);
