...
popup-field=sujet|text|Sujet de la page|||||yes|Ex : Ma nouvelle page|60|3|Titre affiché en haut de la page
...
| `popup-field=nom\|type\|label\|options\|show-if\|default\|separator\|required\|placeholder\|maxlength\|minlength\|help` | Ajoute un champ à la popup (voir détail des segments ci-dessous) |
...
### Détail des segments de `popup-field`

Seuls les 3 premiers segments (`nom`, `type`, `label`) sont obligatoires. Tous les suivants sont optionnels — tu peux t'arrêter à n'importe lequel, ou laisser un segment vide (`||`) pour passer au suivant sans le renseigner.

| # | Segment | Rôle |
|---|---|---|
| 1 | `nom` | Nom interne du champ (utilisé dans `show-if`, les variables `$1`/`$2`...) |
| 2 | `type` | `text`, `textarea`, `select`, `radio`, `checkbox` |
| 3 | `label` | Libellé affiché au-dessus du champ |
| 4 | `options` | Liste `A,B,C` pour `select`/`radio`/`checkbox`, ou valeur initiale pour `text`/`textarea` |
| 5 | `show-if` | Condition d'affichage, ex. `show-if:type=article` |
| 6 | `default` | Valeur pré-remplie (prioritaire sur `options` pour `text`/`textarea`) |
| 7 | `separator` | Séparateur utilisé pour joindre les cases cochées d'un `checkbox` multiple (défaut `, `) |
| 8 | `required` | `yes` pour rendre le champ obligatoire avant publication |
| 9 | `placeholder` | Texte indicatif affiché dans un champ `text`/`textarea` vide |
| 10 | `maxlength` | Nombre maximal de caractères autorisés (`text`/`textarea`) |
| 11 | `minlength` | Nombre minimal de caractères requis (`text`/`textarea`) |
| 12 | `help` | Texte d'aide affiché via une bulle d'info à côté du champ |

Exemple avec les nouveaux paramètres :

```
popup-field=titre|text|Titre de la page|||||yes|Ex : Ma nouvelle page|60|3|Un titre court et explicite, sans ponctuation finale
```

Ici : pas d'`options`, pas de `show-if`, pas de `default`, séparateur par défaut, champ obligatoire, un placeholder, entre 3 et 60 caractères, et un texte d'aide.
